...
MainWindow::MainWindow(QString filePath, QString token, QString course_id,
                       QString video_id, QString video_item_id,
                       QString identifier, QString deviceId, QWidget *parent)
    : QMainWindow(parent),
  ...
  connect(manager, &ApiManager::onKeyFetchFinished, this,
          &MainWindow::onKeyFetchCompleted);
  ...
  connect(Player, &VLCPlayer::positionChanged, this,
          &MainWindow::positionChanged);
  // connect(Player, &QMediaPlayer::positionChanged, this,
  // &MainWindow::positionChanged); no change connect(Player,
  // &QMediaPlayer::mediaStatusChanged, this, &MainWindow::loadVideo);
  connect(Player, &VLCPlayer::mediaStatusChanged, this, &MainWindow::loadVideo);

  // connect(Player, &QMediaPlayer::playbackStateChanged, this,
  // &MainWindow::handlePlayPauseButtonState);
  connect(Player, &VLCPlayer::playPausedStatusChanged, this,
          &MainWindow::handlePlayPauseButtonState);

  seekbarController =
      new VideoProgressBarController(ui->horizontalSlider_Duration, 0);
  seekbarController->setSliderMaxLimit(
      getSumOfAllVideosTimeTillNow(videoItemList.count() - 1));
  seekbarConnection = connect(
      seekbarController, &VideoProgressBarController::onSeekbarStopedSliding,
      this, &MainWindow::onSliderStop);
  connect(ui->horizontalSlider_Duration, &CustomSeekbar::valueChanged, this,
          &MainWindow::on_horizontalSlider_Duration_sliderMoved);

  connect(ui->horizontalSlider_Duration, &CustomSeekbar::userClickOnSeekbar,
          [this]() { this->userAction = true; });
  //    connect(ui->horizontalSlider_Duration,&CustomSeekbar::actionTriggered,this,&MainWindow::slderClicked);
  connect(ui->horizontalSlider_Volume, &QSlider::valueChanged, this,
          &MainWindow::handleVolumeChange);
  connect(playbackRateHandler, &PlaybackRateHandler::playbackRateChanged, this,
          &MainWindow::onPlaybackRateChanged);

  connect(manager, &ApiManager::noNetworkForTimer, this,
          &MainWindow::userPlaytimeDataFailed);
  connect(manager, &ApiManager::onUserTimeSentSuccess, this,
          &MainWindow::userPlaytimeDataSuccess);
  ...
  // connect(playbackTimer, &UserPlaybackTimerTracker::sendMetrics,
  // this,&MainWindow::sendTimeToServer);
...
void MainWindow::sendTimeToServer(qint64 playTimeInSeconds) {
  ...
  this->manager->sendUserWatchTime(token, course_id, video_item_id, video_id,
...
void MainWindow::slderClicked(int action) {
  // qDebug()<<"slider clicked called"<<action<<"slider
  // value"<<seekbarController->getValue();
  //  if(IS_FULL_SCREEN){
...
void MainWindow::durationChanged(qint64 duration) {
  ...
  if (IS_FULL_SCREEN && fsSeekbar != nullptr) {
    ...
    // seekbarController->setSliderMaxLimit(getSumOfAllVideosTimeTillNow(videoItemList.count()
    // - 1));
...
void MainWindow::positionChanged(qint64 duration) {
  ...
  if ((QDateTime::currentSecsSinceEpoch() - lastDisplayTime > 1)) {
    ...
    /// TODO: come back here
    watermarkHandler->updateWatermark(ui->video_section->width(),
...
void MainWindow::onlyUpdatePlaybackTimeText(QString playbackDurationString) {
  ...
  QTime TotalTime((fullVideoDurtaion / 3600) % 60,
                  (fullVideoDurtaion / 60) % 60, fullVideoDurtaion % 60,
                  (fullVideoDurtaion * 1000) % 1000);
  QString Format = "";
  if (fullVideoDurtaion > 3600)
    Format = "hh:mm:ss";
  else
    Format = "mm:ss";
  if (IS_FULL_SCREEN && fsSeekbar != nullptr && fsTotalTime != nullptr &&
      fsCurrentTime != nullptr) {
...
void MainWindow::updateDuration(qint64 Duration) {
  ...
  if (Duration || fullVideoDurationLocal) {
    ...
    QTime TotalTime((videoDurationInSeconds / 3600) % 60,
                    (videoDurationInSeconds / 60) % 60,
                    videoDurationInSeconds % 60,
                    (videoDurationInSeconds * 1000) % 1000);
    QString Format = "";
...
void MainWindow::on_actionOpen_triggered(MizuConfig *config) {
  ...
  watermarkHandler = new MizuShirushiHandora(this, wMarkScreen,
                                             this->identifier, 3, 3, config);
  ...
  watermarkHandler->postionPrimary(ui->centralwidget->width(),
  ...
  RECORDING_RED_DOT =
      new RedDotRecording(wMarkScreen, this->identifier.toStdString(),
                          config->dflConfig, ui->video_section);
  RECORDING_FLASH_LAYER = new ScreenFlashLayer(
      wMarkScreen, phoneNumber, config->flConfig, ui->video_section);
  // Player->setVideoOutput(videoItem);
  RECORDING_RED_DOT->updatePosition(20, wMarkScreen->height());
  RECORDING_FLASH_LAYER->updateSize(0, 0, ui->centralwidget->width(),
                                    ui->centralwidget->height());
  ...
  ui->video_section->setGeometry(0, 0, ui->video_section->width(),
                                 ui->video_section->height());
  // videoItem->setSize(QSize(ui->video_section->width(),
  // ui->video_section->height()));
  ...
  seekbarController =
      new VideoProgressBarController(ui->horizontalSlider_Duration, fileCount);
  connect(seekbarController,
          &VideoProgressBarController::onSeekbarSecondsTimerEndSliding, this,
          &MainWindow::seekToRemainingTime);
  seekbarController->setSliderMaxLimit(
      getSumOfAllVideosTimeTillNow(videoItemList.count() - 1));
  seekbarConnection = connect(
      seekbarController, &VideoProgressBarController::onSeekbarStopedSliding,
  ...
  QFile uiFile(
      ":/controls_overlay.ui");  // Ensure the path to your .ui file is correct
  ...
  connect(controlsWrapper, &FullScreenControlsWidget::exitFullScreen,
          [this]() { this->on_normal_button_pressed(); });

  connect(wMarkScreen, &CustomGestureWidget::makeFullScreen,
          [this]() { this->on_pushButton_full_screen_clicked(); });
  // controls->hide();
}
...
void MainWindow::on_pushButton_Play_Pause_clicked() {
  ...
  if (Player->playbackState() == PlaybackState::Playing) {
    ...
    if (IS_FULL_SCREEN) {
      int videoIndexToJump =
          getVideoIndexToJump(fsSeekbarController->getValue());
...
int MainWindow::getSumOfAllVideosTimeTillNow(int index) {
  qint64 fullVideoDurtaion = 0;
  for (int i = 0; (i < videoTimeArray.length()) &&
                  (index < videoTimeArray.size()) && (index >= 0);
       i++) {
    fullVideoDurtaion += videoTimeArray[i] / 1000;
  }
...
void MainWindow::loadVideo(PlaybackState status) {
  ...
  if (status == PlaybackState::Changed && !isPaused && mediaStopped == false &&
      !blockedForPiracy && !pausedForNoInternet) {
    // Player->play();
    qDebug() << "media changed";
    caliberateVideo();
  }

  // if(status == QMediaPlayer::MediaStatus::LoadedMedia && !isPaused &&
  // mediaStopped == false && !blockedForPiracy && !pausedForNoInternet){
  //     Player->play();
  //     caliberateVideo();
  // }
...
void MainWindow::on_horizontalSlider_Duration_sliderMoved() {
  ...
  if (IS_FULL_SCREEN) {
    if (fsSeekbarController->horizontalSlider_Duration->isSliderDown() ||
        userAction) {
      fsSeekbarController->setupTimer(this);
      seekbarNewController->markGettingDragged();
    }
  } else {
    if (seekbarController->horizontalSlider_Duration->isSliderDown() ||
        userAction) {
...
int MainWindow::getExtraSeek(int timeInSeconds, int indexToJump) {
  ...
  if ((sumToIndex / 1000) >=
      seekbarController->horizontalSlider_Duration->maximum()) {
...
void MainWindow::setupFullScreenControls() {
  try {
    ...
    controls->setGeometry(QRect(0, ui->centralwidget->height() - 200,
                                ui->centralwidget->width(), 200));

    if (fsPlayPauseButton == nullptr) {
      fsPlayPauseButton = controls->findChild<QPushButton *>(
          "fs_PushButton_Play_Pause",
          Qt::FindChildOption::FindChildrenRecursively);
      connect(fsPlayPauseButton, &QPushButton::clicked, this,
              &MainWindow::on_pushButton_Play_Pause_clicked);
    }
    if (fsStopButton == nullptr) {
      fsStopButton = controls->findChild<QPushButton *>(
          "fs_PushButton_Stop", Qt::FindChildOption::FindChildrenRecursively);
    }
    if (fsSeekbar == nullptr) {
      fsSeekbar = qobject_cast<CustomSeekbar *>(controls->findChild<QSlider *>(
          "fs_HorizontalSlider_Duration",
          Qt::FindChildOption::FindChildrenRecursively));
    }
    if (fsNormalButton == nullptr) {
      fsNormalButton = controls->findChild<QPushButton *>(
          "normal_screen_button", Qt::FindChildOption::FindChildrenRecursively);
    }
    if (fsTotalTime == nullptr) {
      fsTotalTime = controls->findChild<QLabel *>(
          "fs_label_total_time", Qt::FindChildOption::FindChildrenRecursively);
    }
    if (fsCurrentTime == nullptr) {
      fsCurrentTime = controls->findChild<QLabel *>(
          "fs_label_current_time",
          Qt::FindChildOption::FindChildrenRecursively);
    }
    if (fsTenSecBackward == nullptr) {
      fsTenSecForward = controls->findChild<QPushButton *>(
          "fs_PushButton_Seek_Forward",
          Qt::FindChildOption::FindChildrenRecursively);
    }
    if (fsTenSecBackward == nullptr) {
      fsTenSecBackward = controls->findChild<QPushButton *>(
          "fs_PushButton_Seek_Backward",
          Qt::FindChildOption::FindChildrenRecursively);
    }
    if (fsSpeed1x == nullptr) {
      fsSpeed1x = controls->findChild<QPushButton *>(
          "pushButton_1x", Qt::FindChildOption::FindChildrenRecursively);
    }
    if (fsSpeed1p2x == nullptr) {
      fsSpeed1p2x = controls->findChild<QPushButton *>(
          "pushButton_1p2x", Qt::FindChildOption::FindChildrenRecursively);
    }
    if (fsSpeed1p5x == nullptr) {
      fsSpeed1p5x = controls->findChild<QPushButton *>(
          "pushButton_1p5x", Qt::FindChildOption::FindChildrenRecursively);
    }
    if (fsSpeed2x == nullptr) {
      fsSpeed2x = controls->findChild<QPushButton *>(
          "pushButton_2x", Qt::FindChildOption::FindChildrenRecursively);
    }
    if (fsSpeedBox == nullptr) {
      fsSpeedBox = controls->findChild<QComboBox *>(
          "comboBoxSpeed", Qt::FindChildOption::FindChildrenRecursively);
    }
    if (fsSeekbarVolume == nullptr) {
      fsSeekbarVolume = controls->findChild<QSlider *>(
          "fs_HorizontalSlider_Volume",
          Qt::FindChildOption::FindChildrenRecursively);
    }
    // Connect the buttons to the media player
    if (fsVolumeLowIcon == nullptr) {
      fsVolumeLowIcon = controls->findChild<QLabel *>(
          "label_2", Qt::FindChildOption::FindChildrenRecursively);
    }
    if (fsPlaybackLabel == nullptr) {
      fsPlaybackLabel = controls->findChild<QLabel *>(
          "label_3", Qt::FindChildOption::FindChildrenRecursively);
    }
    ...
    /////
    /// \brief connect
    // FullScreenControlHoverHandler* hoverArea = new
    // FullScreenControlHoverHandler(proxy, QRectF(0,ui->centralwidget->height()
    // - 200,ui->centralwidget->width(),200)); hoverArea->setPen(Qt::NoPen);
    ...
    // connect(fsPlayPauseButton, &QPushButton::clicked, this,
    // &MainWindow::on_pushButton_Play_Pause_clicked);
    connect(fsStopButton, &QPushButton::clicked, this,
            &MainWindow::on_pushButton_Stop_clicked);
    connect(fsNormalButton, &QPushButton::clicked, this,
            &MainWindow::on_normal_button_pressed);
    ///

    ///////
    fsSeekbarController = new VideoProgressBarController(fsSeekbar, fileCount);
    fsSeekbarController->setSliderMaxLimit(
        getSumOfAllVideosTimeTillNow(videoItemList.count() - 1));
    fsSeekbar->setTracking(false);
    fsSeekbarConnection =
        connect(fsSeekbarController,
                &VideoProgressBarController::onSeekbarStopedSliding, this,
                &MainWindow::onSliderStop);
    fsSeekbarForwardBackwardConnection =
        connect(fsSeekbarController,
                &VideoProgressBarController::onSeekbarSecondsTimerEndSliding,
                this, &MainWindow::seekToRemainingTime);
    connect(fsSeekbar, &CustomSeekbar::valueChanged, this,
            &MainWindow::on_horizontalSlider_Duration_sliderMoved);
    // connect(fsSeekbar,&CustomSeekbar::actionTriggered,this,&MainWindow::slderClicked);
    connect(fsTenSecForward, &QPushButton::clicked, this,
            &MainWindow::on_pushButton_Seek_Forward_clicked);
    connect(fsTenSecBackward, &QPushButton::clicked, this,
            &MainWindow::on_pushButton_Seek_Backward_clicked);
    connect(fsSeekbar, &CustomSeekbar::userClickOnSeekbar,
            [this]() { this->userAction = true; });
    handlePlayPauseButtonState(Player->playPauseState());
    connect(fsSpeed1x, &QPushButton::clicked, this,
            &MainWindow::on_pushButton_1x_clicked);

    connect(fsSpeed1p2x, &QPushButton::clicked, this,
            &MainWindow::on_pushButton_1p2x_clicked);
    connect(fsSpeed1p5x, &QPushButton::clicked, this,
            &MainWindow::on_pushButton_1p5x_clicked);
    connect(fsSpeed2x, &QPushButton::clicked, this,
            &MainWindow::on_pushButton_2x_clicked);
    connect(fsSpeedBox, &QComboBox::currentTextChanged, this,
    ...
    connect(fsSeekbarVolume, &QSlider::valueChanged, this,
            &MainWindow::on_horizontalSlider_Volume_valueChanged);
    RECORDING_RED_DOT->updatePosition(20, ui->video_section->height() - 40);
    RECORDING_FLASH_LAYER->updateSize(0, 0, ui->video_section->width(),
                                      ui->video_section->height());
...
void MainWindow::on_normal_button_pressed() {
  try {
    ...
    // videoItem->setSize(QSize(ui->video_section->width(),
    // ui->video_section->height())); ui->video_section->fit
    //  ui->video_section->fitInView(videoItem, Qt::KeepAspectRatio);
    //  ui->video_section->setGeometry(0, 0, ui->video_section->width(),
    //  ui->video_section->height());
    update();
    // ui->video_section->setAlignment(Qt::AlignCenter);
    // ui->video_section->centerOn(view->mapToScene(
...
// void MainWindow::handlePlayPauseButtonState(QMediaPlayer::PlaybackState
// playbackState){
...
void MainWindow::on_pushButton_full_screen_clicked() {
  ...
  // videoItem->setSize(QSize(ui->centralwidget->width(),
  // ui->centralwidget->height())); view->fitInView(videoItem,
  // Qt::KeepAspectRatio); view->setGeometry(0, 0, ui->centralwidget->width(),
  // ui->centralwidget->height());
  ...
  // ui->centralwidget->height())); view->fitInView(videoItem,
  // Qt::KeepAspectRatio); view->setGeometry(0, 0, ui->centralwidget->width(),
  // ui->centralwidget->height()); WATERMARK_TEXT->setPos(100,100);
...
void MainWindow::handleUserManualFullScreen() {
  ...
  ui->controlsSection->hide();
  // videoItem->setSize(QSize(ui->centralwidget->width(),
  // ui->centralwidget->height())); ui->video_section->fitInView(videoItem,
  // Qt::KeepAspectRatio); ui->video_section->setGeometry(0, 0,
  // ui->centralwidget->width(), ui->centralwidget->height());
  watermarkHandler->updateWatermark(ui->centralwidget->width(),
                                    ui->centralwidget->height());
...
void MainWindow::handleUserManualUnMaximize() {
  ...
  // videoItem->setSize(QSize(ui->centralwidget->width(),
  // ui->centralwidget->height()));
  ui->video_section->setGeometry(0, 0, ui->centralwidget->width(),
                                 ui->centralwidget->height());
  // videoItem->setSize(QSize(ui->centralwidget->width(),
  // ui->centralwidget->height()));
  ui->video_section->setGeometry(0, 0, ui->video_section->width(),
                                 ui->video_section->width());
...
bool MainWindow::GetMonitorSizeFromEDID(const HKEY hDevRegKey, short &WidthMm,
                                        short &HeightMm) {
  ...
  for (LONG i = 0, retValue = ERROR_SUCCESS; retValue != ERROR_NO_MORE_ITEMS;
       ++i) {
    retValue = RegEnumValue(hDevRegKey, i, valueName, &ActualValueNameLength,
                            NULL, &dwType, EDIDdata, &edidsize);

    if (retValue != ERROR_SUCCESS ||
        QString::fromWCharArray(valueName) != "EDID")
      continue;
...
bool MainWindow::GetSizeForDevID(short &WidthMm, short &HeightMm) {
  HDEVINFO devInfo = SetupDiGetClassDevsEx(&GUID_CLASS_MONITOR, NULL, NULL,
...
void MainWindow::onKeyFetchCompleted(QList<VideoData> keyList,
  ...
  this->playbackTimer =
      new UserPlaybackTimerTracker(Player, config->log_activity_interval, this);
  // Connect to the play time updated signal
  connect(playbackTimer, &UserPlaybackTimerTracker::sendMetrics, this,
  ...
  if ((allMonitors.length() == 1 && monitorCount == 1) || ( config->detect_external_display == false)) {
    ...
    durationAverageInSeconds =
        videoItemList.size() == 1
            ? getSumOfAllVideosTimeTillNow(videoItemList.size() - 1)
            : getSumOfAllVideosTimeTillNow(videoItemList.size() - 1) /
                      videoItemList.size() -
                  1;
    // durationAverageInSeconds = ;
    ...
    QTimer::singleShot(0, this, [this]() { showWarningDialog(); });
  }
}
...
void MainWindow::onNoInternet(QString message) {
  ...
  noIntentDialog->show();
  connect(noIntentDialog, &NoInternetDialog::onNoInternetDialogClose, this,
          &MainWindow::closeNoInternetDialogAndRetry);
}
...
void MainWindow::userPlaytimeDataFailed(QString message) {
      ...
      QTimer::singleShot(this->retryInterval, this, [this]() {
        ...
        if (pendingItems.length() > 0) {
          ...
          manager->sendUserWatchTime(token, course_id, video_item_id, video_id,
                                     lastPendingItem, currentSeekbarPostion);
          this->retryCounter--;
        }
      });
    } else {
  ...
  connect(noIntentDialogForTimer, &NoInternetDialog::onNoInternetDialogClose,
          this, &MainWindow::closeWatchTimeNoInternetDialogAndRetry);
}
...
void MainWindow::userPlaytimeDataSuccess() {
  ...
  if (Player->playPauseState() == PlayPauseState::Pause &&
      pausedForNoInternet) {
    Player->play();
  ...
  ScreenDetector detector = ScreenDetector();
  if (monitorSizes.length() > 1 ||
      detector.getMonitorInfoFromDeviceManager() > 1) {
    QTimer::singleShot(0, this, [this]() { showWarningDialog(); });
  }
}
