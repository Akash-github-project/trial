...
private:
    void EncryptBlock();
    void DecryptBlock();
    void KeyExpansion(const deque<Byte>& key);

    void AddRoundKey(deque<deque<Byte>>& block, const size_t shift = 0) const;
    void SubBytes(deque<deque<Byte>>& block) const;
    void ShiftRows(deque<deque<Byte>>& block) const;
    void MixColumns(deque<deque<Byte>>& block) const;
    void InvSubBytes(deque<deque<Byte>>& block) const;
    void InvShiftRows(deque<deque<Byte>>& block) const;
    void InvMixColumns(deque<deque<Byte>>& block) const;

    void RotWord(deque<Byte>& a) const;
    void SubWord(deque<Byte>& a) const;
    void Rcon(deque<Byte>& a, size_t n) const;
    void XorWords(const deque<Byte> a, const deque<Byte> b, deque<Byte>& c) const;
    Byte mulBytes(Byte a, Byte b) const;

    void ShiftRow(deque<Byte>& row, int n) const;
    void MixSingleColumn(deque<Byte>& r) const;
private:
    size_t Nb;
    size_t Nk;
    size_t Nr;
    Byte blockLen;
    deque<Byte> inBlock;
    deque<Byte> roundKeys;
};

const deque<Byte> sBox{
